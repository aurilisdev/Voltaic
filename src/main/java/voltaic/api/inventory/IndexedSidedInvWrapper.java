...
public class IndexedSidedInvWrapper implements IItemHandlerModifiable {
	...
	this.newStackInsertLimit = (wrapperSlot, invSlot, stack) -> Math.min(stack.getMaxStackSize(),
		getSlotLimit(wrapperSlot));
	...
	if (!stackInSlot.isEmpty()) {
	    if ((stackInSlot.getCount() >= Math.min(stackInSlot.getMaxStackSize(), getSlotLimit(slot))) || !ItemStack.isSameItemSameTags(stack, stackInSlot) || !inv.canPlaceItemThroughFace(slot1, stack, side) || !inv.canPlaceItem(slot1, stack))
		return stack;
