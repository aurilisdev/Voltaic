...
public class GasHandlerItemStack implements IGasHandlerItem, ICapabilityProvider {
	...
	GasStack.CODEC.encode(gas, NbtOps.INSTANCE, NbtOps.INSTANCE.empty()).result()
		.ifPresent(tag -> container.getOrCreateTag().put(GAS_NBT_KEY, tag));
    }

    @Override
    public GasStack getGasInTank(int tank) {

	return GasStack.CODEC.decode(NbtOps.INSTANCE, container.getOrCreateTag().get(GAS_NBT_KEY)).result()
		.orElseGet(() -> Pair.of(GasStack.EMPTY, new CompoundTag())).getFirst();
    }
	...
	public SwapEmpty(ItemStack container, ItemStack emptyContainer, int capacity, int maxTemperature,
		int maxPressure) {
	    super(container, capacity, maxTemperature, maxPressure);
