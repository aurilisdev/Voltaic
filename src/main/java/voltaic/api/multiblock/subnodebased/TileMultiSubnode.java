...
public class TileMultiSubnode extends GenericTile {
    ...
    public final SingleProperty<BlockPos> parentPos = property(
	    new SingleProperty<>(PropertyTypes.BLOCK_POS, "nodePos", BlockEntityUtils.OUT_OF_REACH));
    public final SingleProperty<Integer> nodeIndex = property(
	    new SingleProperty<>(PropertyTypes.INTEGER, "nodeIndex", 0));
	...
	if (level.getBlockEntity(parentPos.getValue()) instanceof IMultiblockParentTile node) {
	    ...
	    return shapeCache = node.getSubNodes().getSubnodes(node.getFacingDirection())[nodeIndex.getValue()]
		    .getShape(node.getFacingDirection());
