 ...
 * @author skip999
 *
 * @param placeState    : The blockstate that will be placed when the slave node
 *                      is created by the controller. This is to allow for
 *                      extensions of the base TileMultiblockSlave class
 * @param replaceState  : The blockstate the controller will look for for valid
 *                      placement. Note if using tags, it will look for
 *                      Properties after checking the tag
 * @param taggedBlocks: A list of valid blocks the controller will look for.
 *                      Leave null if there is only one valid block, which will
 *                      be represented by replacedState
 ...
 *
 */
public record MultiblockSlaveNode(BlockState placeState, BlockState replaceState, TagKey<Block> taggedBlocks,
	Vec3i offset, VoxelShape renderShape, ResourceLocation model) {
    ...
    public static final Codec<MultiblockSlaveNode> CODEC = RecordCodecBuilder.create(instance -> instance.group(
	    ...
	    //
	    TagKey.codec(Registries.BLOCK).optionalFieldOf(BLOCK_TAG_FIELD, NOTAG)
		    .forGetter(MultiblockSlaveNode::taggedBlocks),
	    //
    ...
    public static final MultiblockSlaveNode EMPTY = new MultiblockSlaveNode(Blocks.AIR.defaultBlockState(),
	    Blocks.AIR.defaultBlockState(), NOTAG, Vec3i.ZERO, Shapes.empty(), NOMODEL);
