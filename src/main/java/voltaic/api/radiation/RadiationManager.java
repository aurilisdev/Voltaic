...
public class RadiationManager implements IRadiationManager, ICapabilitySerializable<CompoundTag> {
	...
	CompoundTag fading = nbt.getCompound("fadingradiationsources");
	...
	for (LivingEntity living : entities) {
	    if (!living.isAlive() || living.getLevel() != world || !living.getBoundingBox().intersects(sourceBounds)) {
		continue;
	    }
	...
	if (refresh) {
	    ...
	    if (newEntry) {
		/*
		 * Spread newly created cache entries over the next EXPOSURE_CACHE_INTERVAL
		 * ticks so that several radiation sources do not all refresh on the same tick.
		 */
	 ...
	 * introduced.
	 */
	long sampleCycle = gameTime / EXPOSURE_CACHE_INTERVAL;
	...
	if (refresh) {
		...
		cached.nextRefreshTick = gameTime + ENTITY_CACHE_INTERVAL;
	    }
	}
