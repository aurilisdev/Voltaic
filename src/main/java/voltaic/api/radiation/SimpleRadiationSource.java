...
public class SimpleRadiationSource implements IRadiationSource {
    ...
    public static final SimpleRadiationSource NONE = new SimpleRadiationSource(0, 0, 0, false, 0,
	    BlockEntityUtils.OUT_OF_REACH, false, false);
    ...
    public static final Codec<SimpleRadiationSource> CODEC = RecordCodecBuilder.create(instance -> instance.group(
	    ...
	    BlockPos.CODEC.optionalFieldOf("location", BlockEntityUtils.OUT_OF_REACH)
		    .forGetter(SimpleRadiationSource::location),
	...
	public SimpleRadiationSource decode(ByteBuf buffer) {
	    return new SimpleRadiationSource(StreamCodec.DOUBLE.decode(buffer), StreamCodec.DOUBLE.decode(buffer),
		    StreamCodec.INT.decode(buffer), StreamCodec.BOOL.decode(buffer), StreamCodec.INT.decode(buffer),
		    StreamCodec.BLOCK_POS.decode(buffer), StreamCodec.BOOL.decode(buffer),
    ...
    public SimpleRadiationSource(double amount, double strength, int distance, boolean isTemporary, int ticks,
	    BlockPos location, boolean shouldLinger, boolean shouldCombine) {
	this.amount = amount;
