...
public class CapabilityRadiationRecipient implements IRadiationRecipient, ICapabilitySerializable<CompoundTag> {
	...
	if (count < 4) {
		...
		if (instance.getAmplifier() > amplitude) {
		    entity.addEffect(new MobEffectInstance(VoltaicEffects.RADIATION.get(),
			    time + instance.getDuration(), instance.getAmplifier(), false, true));
		} else {
		    entity.addEffect(new MobEffectInstance(VoltaicEffects.RADIATION.get(),
			    time + instance.getDuration(), amplitude, false, true));
		}
