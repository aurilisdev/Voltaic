...
public class DistanceSound extends AbstractTickableSoundInstance {
	...
	if (block == null || Minecraft.getInstance().player.distanceToSqr(x + 0.5, y + 0.5, z + 0.5) > 16 * 16
		|| Minecraft.getInstance().level
			.getBlockState(new BlockPos((int) Math.floor(x), (int) Math.floor(y), (int) Math.floor(z)))
			.getBlock() != block) {
	    block = null;
	    return 0;
	}
	return super.getVolume() * (float) Math.min(1.0,
		1.0f / Math.sqrt(Minecraft.getInstance().player.distanceToSqr(x + 0.5, y + 0.5, z + 0.5)));
    }
