...
public interface IItemElectric {
	...
	return new ElectricItemData(0, item.getElectricProperties().capacity,
		item.getElectricProperties().extract.getVoltage(), item.getElectricProperties().receive.getJoules(),
		item.getElectricProperties().extract.getJoules(), new ItemStack(item.getDefaultStorageBattery()));
    }
	...
	ElectricItemData data = stack.getOrDefault(VoltaicDataComponentTypes.ELECTRIC_ITEM_DATA,
		defaultData((IItemElectric) stack.getItem()));
	...
	return item.getOrDefault(VoltaicDataComponentTypes.ELECTRIC_ITEM_DATA,
		defaultData((IItemElectric) item.getItem())).maxJoules;
    }

    static void setMaximumCapacity(ItemStack item, double amt) {
	ElectricItemData data = item.getOrDefault(VoltaicDataComponentTypes.ELECTRIC_ITEM_DATA,
		defaultData((IItemElectric) item.getItem()));
	item.set(VoltaicDataComponentTypes.ELECTRIC_ITEM_DATA, data.setMaxJoules(amt));
    }

    default double getReceiveLimit(ItemStack item) {
	return item
		.getOrDefault(VoltaicDataComponentTypes.ELECTRIC_ITEM_DATA, defaultData((IItemElectric) item.getItem()))
		.recieveCap();
    }
	...
	return item.getOrDefault(VoltaicDataComponentTypes.ELECTRIC_ITEM_DATA,
		defaultData((IItemElectric) item.getItem())).extractCap;
    }
    ...
    /**
     * finds the first battery of matching voltage for this item and swaps out the
     * battery in the item with a new one
     *
     * @param tool
     * @param player
     */
	...
	for (int i = 0; i < inv.items.size(); i++) {
	    ...
	    if (!playerItem.isEmpty() && playerItem.getItem() instanceof IItemElectric electric
		    && electric.isEnergyStorageOnly() && electric.getJoulesStored(playerItem) > 0
		    && electric.getElectricProperties().extract
			    .getVoltage() == electricItem.getElectricProperties().extract.getVoltage()
		...
		for (int j = 0; j < inv.items.size(); j++) {
			...
			player.level().playSound(null, player.getOnPos(), VoltaicSounds.SOUND_BATTERY_SWAP.get(),
	...
	tooltip.add(
		VoltaicTextUtils
			.tooltip("currbattery", ((IItemElectric) stack.getItem()).getCurrentBattery(stack)
				.getDisplayName().copy().withStyle(ChatFormatting.GRAY))
			.withStyle(ChatFormatting.DARK_GRAY));
    }

    public static record ElectricItemData(double joulesStored, double maxJoules, double voltage, double recieveCap,
	    double extractCap, ItemStack battery) {
...
//
	...
	public static final StreamCodec<RegistryFriendlyByteBuf, ElectricItemData> STREAM_CODEC = StreamCodec.composite(
		ByteBufCodecs.DOUBLE, instance0 -> instance0.joulesStored, ByteBufCodecs.DOUBLE,
		instance0 -> instance0.maxJoules, ByteBufCodecs.DOUBLE, instance0 -> instance0.voltage,
		ByteBufCodecs.DOUBLE, instance0 -> instance0.recieveCap, ByteBufCodecs.DOUBLE,
		instance0 -> instance0.extractCap, ItemStack.OPTIONAL_STREAM_CODEC, instance0 -> instance0.battery,
