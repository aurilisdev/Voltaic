...
public class ButtonGuidebook extends ScreenComponentButton<ButtonGuidebook> {
	...
	if (isActiveAndVisible() && isHovered()) {
	    ITexture on = type.on;
	    graphics.blit(on.getLocation(), xLocation + guiWidth, yLocation + guiHeight, on.textureU(), on.textureV(),
		    on.textureWidth(), on.textureHeight(), on.imageWidth(), on.imageHeight());
	} else {
	...
	private GuidebookButtonTextures(int textureWidth, int textureHeight, int textureU, int textureV, int imageWidth,
		int imageHeight, String name) {
	    this(textureWidth, textureHeight, textureU, textureV, imageWidth, imageHeight,
		    Voltaic.rl("textures/screen/guidebook/buttons/" + name + ".png"));
	}
