...
public class ScreenComponentProgress extends ScreenComponentGeneric {
	...
	case PROGRESS_ARROW_LEFT:
	    ...
	    graphics.blit(on.getLocation(), guiWidth + xLocation + on.textureWidth() - progress, guiHeight + yLocation,
		    xStart, on.textureV(), progress, on.textureHeight(), on.imageWidth(), on.imageHeight());
	    break;
	case COUNTDOWN_FLAME, FAN:
	    progress = (int) (progressInfoHandler.getAsDouble() * on.textureHeight());
	    graphics.blit(on.getLocation(), guiWidth + xLocation, guiHeight + yLocation + on.textureHeight() - progress,
		    on.textureU(), on.textureV() + on.textureHeight() - progress, on.textureWidth(), progress,
		    on.imageWidth(), on.imageHeight());
	    break;
	case PROGRESS_ARROW_RIGHT, BATTERY_CHARGE_RIGHT, PROGRESS_ARROW_RIGHT_BIG:
	    graphics.blit(on.getLocation(), guiWidth + xLocation, guiHeight + yLocation, on.textureU(), on.textureV(),
		    (int) (progressInfoHandler.getAsDouble() * on.textureWidth()), on.textureHeight(), on.imageWidth(),
		    on.imageHeight());
	    break;
	default:
	    break;
	...
	ProgressTextures(int textureWidth, int textureHeight, int textureU, int textureV, int imageWidth,
		int imageHeight, String name) {
	    ...
	    loc = Voltaic.rl("textures/screen/component/progress/" + name + ".png");
	}
