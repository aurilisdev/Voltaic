...
/**
 * 
 * A modified variant of AbstractWidget aligning it more with the system
 * Electrodynamics uses as AbstractWidget has several issues
 * 
 * @author skip999
 *
...
public abstract class AbstractScreenComponent implements GuiEventListener, Renderable, NarratableEntry {
    ...
    /*
     * By separating the GUI definition from the constructor, it allows us to define
     * screen components that can be attached to a screen at a later point. This is
     * especially useful in GUIs that have components that do not change and thus
     * can be made static to improve efficiency
     * 
     ...
     * button) { if (isActiveAndVisible() && isValidClick(button) &&
     * isInClickRegion(mouseX, mouseY)) {
     * 
     * onMouseClick(mouseX, mouseY);
     * 
     * return true; } return false; }
     * 
     * @Override public boolean mouseReleased(double mouseX, double mouseY, int
     * button) { if (isValidClick(button)) { onMouseRelease(mouseX, mouseY); return
     * true; } else { return false; } }
     * 
     * @Override public boolean mouseDragged(double mouseX, double mouseY, int
     * button, double dragX, double dragY) { if (isValidClick(button)) {
     * onMouseDrag(mouseX, mouseY, dragX, dragY); return true; } else { return
     * false; } }
     * 
     * @Override public boolean mouseScrolled(double mouseX, double mouseY, double
     * delta) { if(isActiveAndVisible()) { onMouseScroll(mouseX, mouseY, delta);
