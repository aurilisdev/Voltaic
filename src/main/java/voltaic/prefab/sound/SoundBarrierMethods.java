...
public class SoundBarrierMethods {
	...
	public static <T extends BlockEntity & ITickableSound> void playTileSound(SoundEvent event, T tile, double range, boolean repeat) {
	...
	public static <T extends BlockEntity & ITickableSound> void playTileSound(SoundEvent event, SoundSource source, T tile, float volume, float pitch, double range, boolean repeat) {
		Minecraft.getInstance().getSoundManager().play(new TickableSoundTile<>(event, source, tile, volume, pitch, range, repeat));
	}
