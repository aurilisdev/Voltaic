...
public class ComponentForgeEnergy implements IComponent {
	...
	ICapabilityElectrodynamic electrodynamic = electro
		.getCapability(VoltaicCapabilities.CAPABILITY_ELECTRODYNAMIC_BLOCK, side, inputType)
		.orElse(CapabilityUtils.EMPTY_ELECTRO);

	return electrodynamic == CapabilityUtils.EMPTY_ELECTRO ? LazyOptional.empty()
		: LazyOptional.of(() -> new ElectrodynamicWrapper(electrodynamic)).cast();
    }
