...
public class ComponentGasHandlerMulti implements IComponentGasHandler {
	...
	if (recipeType != null) {
		...
		for (TagKey<Gas> tag : validInputGasTags) {
		    inputValidatorGases.addAll(VoltaicRegistries.gasRegistry().tags().getTag(tag).stream().toList());
		}
	    }
		...
		for (TagKey<Gas> tag : validOutputGasTags) {
		    outputValidatorGases.addAll(VoltaicRegistries.gasRegistry().tags().getTag(tag).stream().toList());
		}
	    }
	}
