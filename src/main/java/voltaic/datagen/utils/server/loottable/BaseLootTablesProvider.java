...
public abstract class BaseLootTablesProvider extends AbstractLootTableProvider {
    ...
    public <T extends GenericTile> void addMachineTable(Block block, RegistryObject<TileEntityType<T>> tilereg,
	    boolean items, boolean fluids, boolean gases, boolean energy, boolean additional) {
	lootTables.put(block,
		machineTable(name(block), block, tilereg.get(), items, fluids, gases, energy, additional));
    }
    ...
    public <T extends Block, A extends Item> void addFortuneAndSilkTouchTable(RegistryObject<T> reg, A nonSilk,
	    int minDrop, int maxDrop) {
	addFortuneAndSilkTouchTable(reg.get(), nonSilk, minDrop, maxDrop);
    }

    public <T extends Block, A extends Item> void addFortuneAndSilkTouchTable(T block, A nonSilk, int minDrop,
	    int maxDrop) {
