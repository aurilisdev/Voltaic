...
public class CustomShapedCraftingRecipe extends ShapedRecipeBuilder.Result {
    ...
    private CustomShapedCraftingRecipe(ResourceLocation recipeId, Item result, int count, List<String> pattern,
	    Map<Character, Ingredient> keys, ICondition[] recipeConditions) {
	...
	public void complete(String parent, String name, Consumer<FinishedRecipe> consumer) {
	    ...
	    consumer.accept(new CustomShapedCraftingRecipe(new ResourceLocation(parent, name), item, count, patterns,
		    keys, recipeConditions));
	}
