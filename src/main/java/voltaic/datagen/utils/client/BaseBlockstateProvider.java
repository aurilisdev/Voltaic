...
public abstract class BaseBlockstateProvider extends BlockStateProvider {
    ...
    /*
     * private void omniDirBlock(DeferredHolder<Block, Block> block, ModelFile
     * model, boolean registerItem) {
	...
	BlockModelBuilder builder = models().getBuilder(name(block)).customLoader(SlaveNodeModelBuilder::begin)
		.model(existingBlock(VoltaicBlocks.BLOCK_MULTISUBNODE.get())).end()
		.texture("particle", modLoc(particleTexture)).renderType("cutout");
	getVariantBuilder(block).partialState().setModels(new ConfiguredModel(builder));
    }
    ...
    public BlockModelBuilder blockTopBottom(DeferredHolder<Block, ? extends Block> block, String top, String bottom,
	    String side) {
	return models().cubeBottomTop(BuiltInRegistries.BLOCK.getKey(block.get()).getPath(),
		...
		ResourceLocation.fromNamespaceAndPath(modID, top));
    }
