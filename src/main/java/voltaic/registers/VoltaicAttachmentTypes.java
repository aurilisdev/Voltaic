...
public class VoltaicAttachmentTypes {
    ...
    public static final DeferredHolder<AttachmentType<?>, AttachmentType<HashMap<BlockPos, SimpleRadiationSource>>> PERMANENT_RADIATION_SOURCES = ATTACHMENT_TYPES
	    .register("permanentradiationsources", () -> AttachmentType
		    .builder(() -> new HashMap<BlockPos, SimpleRadiationSource>())
		    .serialize(new IAttachmentSerializer<CompoundTag, HashMap<BlockPos, SimpleRadiationSource>>() {
			@Override
			public HashMap<BlockPos, SimpleRadiationSource> read(IAttachmentHolder holder, CompoundTag tag,
				...
				data.put(
					BlockPos.CODEC.parse(new Dynamic<>(NbtOps.INSTANCE, stored.get("pos"))).result()
						.get(),
					SimpleRadiationSource.CODEC
						.parse(new Dynamic<>(NbtOps.INSTANCE, stored.getCompound("radiation")))
						.getOrThrow());
			    }
			...
			@Override
			public @Nullable CompoundTag write(HashMap<BlockPos, SimpleRadiationSource> attachment,
				HolderLookup.Provider provider) {
				...
				BlockPos.CODEC.encodeStart(NbtOps.INSTANCE, entry.getKey())
					.ifSuccess(tag -> store.put("pos", tag));
				SimpleRadiationSource.CODEC.encodeStart(NbtOps.INSTANCE, entry.getValue())
					.ifSuccess(tag -> store.put("radiation", tag));
				data.put(i + "", store);
				i++;
			    }
			    return data;
    ...
    public static final DeferredHolder<AttachmentType<?>, AttachmentType<HashMap<BlockPos, IRadiationManager.TemporaryRadiationSource>>> TEMPORARY_RADIATION_SOURCES = ATTACHMENT_TYPES
	    .register("temporaryradiationsources", () -> AttachmentType
		    .builder(() -> new HashMap<BlockPos, IRadiationManager.TemporaryRadiationSource>()).serialize(
			    new IAttachmentSerializer<CompoundTag, HashMap<BlockPos, IRadiationManager.TemporaryRadiationSource>>() {
				@Override
				public HashMap<BlockPos, IRadiationManager.TemporaryRadiationSource> read(
					...
					CompoundTag stored = tag.getCompound("" + i);
					data.put(
						BlockPos.CODEC.parse(new Dynamic<>(NbtOps.INSTANCE, stored.get("pos")))
							.result().get(),
						IRadiationManager.TemporaryRadiationSource.CODEC.parse(
							new Dynamic<>(NbtOps.INSTANCE, stored.getCompound("radiation")))
							.getOrThrow());
				    }
				...
				public @Nullable CompoundTag write(
					HashMap<BlockPos, IRadiationManager.TemporaryRadiationSource> attachment,
					HolderLookup.Provider provider) {
				    CompoundTag data = new CompoundTag();
				    int size = attachment.size();
				    data.putInt("size", size);
				    int i = 0;
				    for (Map.Entry<BlockPos, IRadiationManager.TemporaryRadiationSource> entry : attachment
					    .entrySet()) {
					CompoundTag store = new CompoundTag();
					BlockPos.CODEC.encodeStart(NbtOps.INSTANCE, entry.getKey())
						.ifSuccess(tag -> store.put("pos", tag));
    ...
    public static final DeferredHolder<AttachmentType<?>, AttachmentType<HashMap<BlockPos, IRadiationManager.FadingRadiationSource>>> FADING_RADIATION_SOURCES = ATTACHMENT_TYPES
	    .register("fadingradiationsources", () -> AttachmentType
		    .builder(() -> new HashMap<BlockPos, IRadiationManager.FadingRadiationSource>()).serialize(
			    new IAttachmentSerializer<CompoundTag, HashMap<BlockPos, IRadiationManager.FadingRadiationSource>>() {
				@Override
				public HashMap<BlockPos, IRadiationManager.FadingRadiationSource> read(
					IAttachmentHolder holder, CompoundTag tag, HolderLookup.Provider provider) {
					...
					data.put(
						BlockPos.CODEC.parse(new Dynamic<>(NbtOps.INSTANCE, stored.get("pos")))
							.result().get(),
						IRadiationManager.FadingRadiationSource.CODEC.parse(
							new Dynamic<>(NbtOps.INSTANCE, stored.getCompound("radiation")))
							.getOrThrow());
				    }
				...
				@Override
				public @Nullable CompoundTag write(
					HashMap<BlockPos, IRadiationManager.FadingRadiationSource> attachment,
					HolderLookup.Provider provider) {
				    CompoundTag data = new CompoundTag();
				    int size = attachment.size();
				    data.putInt("size", size);
				    int i = 0;
				    for (Map.Entry<BlockPos, IRadiationManager.FadingRadiationSource> entry : attachment
					    .entrySet()) {
					CompoundTag store = new CompoundTag();
					BlockPos.CODEC.encodeStart(NbtOps.INSTANCE, entry.getKey())
						.ifSuccess(tag -> store.put("pos", tag));
    ...
    public static final DeferredHolder<AttachmentType<?>, AttachmentType<HashMap<AABB, Double>>> LOCALIZED_DISSIPATIONS = ATTACHMENT_TYPES
	    .register("localizeddissipations", () -> AttachmentType.builder(() -> new HashMap<AABB, Double>())
		    .serialize(new IAttachmentSerializer<CompoundTag, HashMap<AABB, Double>>() {
			@Override
			public HashMap<AABB, Double> read(IAttachmentHolder holder, CompoundTag tag,
				HolderLookup.Provider provider) {
				...
				CompoundTag stored = tag.getCompound("" + i);
				data.put(CodecUtils.AABB_CODEC.parse(new Dynamic<>(NbtOps.INSTANCE, stored.get("pos")))
					.result().get(), stored.getDouble("amount"));
			    }
			...
			@Override
			public @Nullable CompoundTag write(HashMap<AABB, Double> attachment,
				HolderLookup.Provider provider) {
			    CompoundTag data = new CompoundTag();
			    int size = attachment.size();
			    data.putInt("size", size);
				...
				CompoundTag store = new CompoundTag();
				CodecUtils.AABB_CODEC.encodeStart(NbtOps.INSTANCE, entry.getKey())
					.ifSuccess(tag -> store.put("pos", tag));
