...
public class RadioactiveBlockRegister extends AbstractReloadListener<JsonObject> {
    ...
    private final HashMap<INamedTag<Block>, RadioactiveObject> tags = new HashMap<>();
    ...
    protected JsonObject prepare(IResourceManager manager, IProfiler profiler) {
        ...
        List<ResourceLocation> resources = new ArrayList<>(manager.listResources(FOLDER, RadioactiveBlockRegister::isJson));
    ...
    public void generateTagValues() {
        ...
        tags.forEach((tag, value) -> {
            BlockTags.getAllTags().getTag(tag.getName()).getValues().forEach(gas -> {
