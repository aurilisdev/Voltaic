...
public class RadiationShieldingRegister extends SimplePreparableReloadListener<JsonObject> {
	...
	List<Map.Entry<ResourceLocation, Resource>> resources = new ArrayList<>(
		manager.listResources(FOLDER, RadiationShieldingRegister::isJson).entrySet());
	...
	json.entrySet().forEach(set -> {
	    ...
	    RadiationShielding value = RadiationShielding.CODEC.decode(JsonOps.INSTANCE, set.getValue()).getOrThrow()
		    .getFirst();
