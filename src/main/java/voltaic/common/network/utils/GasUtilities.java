package voltaic.common.network.utils;

import javax.annotation.Nullable;

import net.minecraft.core.Direction;
import net.minecraft.sounds.SoundEvents;
import net.minecraft.sounds.SoundSource;
import net.minecraft.world.item.ItemStack;
import net.minecraft.world.level.Level;
import net.minecraft.world.level.block.entity.BlockEntity;
import voltaic.api.gas.GasAction;
import voltaic.api.gas.GasStack;
import voltaic.api.gas.GasTank;
import voltaic.api.gas.IGasHandler;
import voltaic.api.gas.IGasHandlerItem;
import voltaic.prefab.tile.GenericTile;
import voltaic.prefab.tile.components.IComponentType;
import voltaic.prefab.tile.components.type.ComponentInventory;
import voltaic.prefab.utilities.BlockEntityUtils;
import voltaic.registers.VoltaicCapabilities;

public class GasUtilities {

    public static boolean isGasReciever(@Nullable BlockEntity acceptor, Direction dir) {
	if (acceptor == null)
	    return false;

	Level level = acceptor.getLevel();
	if (level == null)
	    return false;

	return level.getCapability(VoltaicCapabilities.CAPABILITY_GASHANDLER_BLOCK, acceptor.getBlockPos(),
		acceptor.getBlockState(), acceptor, dir) != null;
    }

    public static int recieveGas(BlockEntity reciever, Direction dir, GasStack gas, GasAction action) {
	if (gas.isEmpty() || gas.getAmount() <= 0)
	    return 0;

	Level level = reciever.getLevel();
	if (level == null)
	    return 0;

	IGasHandler handler = level.getCapability(VoltaicCapabilities.CAPABILITY_GASHANDLER_BLOCK,
		reciever.getBlockPos(), reciever.getBlockState(), reciever, dir);
	if (handler == null)
	    return 0;

	GasStack copy = gas.copy();
	int taken = 0;

	for (int i = 0; i < handler.getTanks(); i++) {
	    if (!handler.isGasValid(i, copy)) {
		continue;
	    }

	    int received = handler.fill(copy, action);
	    copy.shrink(received);
	    taken += received;
	}

	return taken;
    }

    public static void outputToPipe(GenericTile tile, GasTank[] tanks, Direction... outputDirections) {
	Level level = tile.getLevel();
	if (level == null)
	    return;

	Direction facing = tile.getFacing();
	for (Direction relative : outputDirections) {
	    Direction direction = BlockEntityUtils.getRelativeSide(facing, relative);
	    BlockEntity faceTile = level.getBlockEntity(tile.getBlockPos().relative(direction));
	    if (faceTile == null) {
		continue;
	    }

	    IGasHandler handler = level.getCapability(VoltaicCapabilities.CAPABILITY_GASHANDLER_BLOCK,
		    faceTile.getBlockPos(), faceTile.getBlockState(), faceTile, direction.getOpposite());
	    if (handler == null) {
		continue;
	    }

	    for (GasTank gasTank : tanks) {
		if (gasTank.isEmpty()) {
		    continue;
		}

		for (int i = 0; i < handler.getTanks(); i++) {
		    GasStack tankGas = gasTank.getGas().copy();
		    int amountAccepted = handler.fill(tankGas, GasAction.EXECUTE);

		    GasStack taken = new GasStack(tankGas.getGas(), amountAccepted, tankGas.getTemperature(),
			    tankGas.getPressure());

		    gasTank.drain(taken, GasAction.EXECUTE);
		}
	    }
	}
    }

    public static void drainItem(GenericTile tile, GasTank[] tanks) {
	ComponentInventory inv = tile.<ComponentInventory>getComponent(IComponentType.Inventory).orElse(null);
	if (inv == null)
	    return;

	int cylinderIndex = inv.getInputGasStartIndex();
	int size = inv.getInputGasContents().size();
	if (tanks.length < size)
	    return;

	for (int i = 0; i < size; i++) {
	    int index = cylinderIndex + i;
	    GasTank tank = tanks[i];
	    ItemStack stack = inv.getItem(index);
	    int room = tank.getRoom();

	    if (stack.isEmpty() || room <= 0) {
		continue;
	    }

	    IGasHandlerItem handler = stack.getCapability(VoltaicCapabilities.CAPABILITY_GASHANDLER_ITEM);
	    if (handler == null) {
		continue;
	    }

	    for (int j = 0; j < handler.getTanks(); j++) {
		if (room <= 0) {
		    break;
		}

		GasStack taken = handler.drain(room, GasAction.SIMULATE);
		if (taken.isEmpty() || !tank.isGasValid(taken)) {
		    continue;
		}

		int amountTaken = tank.fill(taken, GasAction.EXECUTE);
		handler.drain(amountTaken, GasAction.EXECUTE);
		room = tank.getRoom();
	    }

	    inv.setItem(index, handler.getContainer());
	}
    }

    public static void fillItem(GenericTile tile, GasTank[] tanks) {
	ComponentInventory inv = tile.<ComponentInventory>getComponent(IComponentType.Inventory).orElse(null);
	if (inv == null)
	    return;

	int cylinderIndex = inv.getOutputGasStartIndex();
	int size = inv.getOutputGasContents().size();
	Level level = tile.getLevel();
	if (level == null || tanks.length < size)
	    return;

	for (int i = 0; i < size; i++) {
	    int index = cylinderIndex + i;
	    GasTank tank = tanks[i];
	    ItemStack stack = inv.getItem(index);

	    if (tank.isEmpty() || stack.isEmpty()) {
		continue;
	    }

	    IGasHandlerItem handler = stack.getCapability(VoltaicCapabilities.CAPABILITY_GASHANDLER_ITEM);

	    if (handler == null) {
		continue;
	    }

	    for (int j = 0; j < handler.getTanks(); j++) {
		if (tank.isEmpty()) {
		    break;
		}

		GasStack gas = tank.getGas();
		if (gas.getTemperature() > handler.getTankMaxTemperature(j)
			|| gas.getPressure() > handler.getTankMaxPressure(j)) {
		    level.playSound(null, tile.getBlockPos(), SoundEvents.GENERIC_EXPLODE.value(), SoundSource.BLOCKS,
			    1.0F, 1.0F);
		}

		int taken = handler.fill(gas, GasAction.EXECUTE);
		tank.drain(taken, GasAction.EXECUTE);
	    }

	    inv.setItem(index, handler.getContainer());
	}
    }
}