...
public class CountableIngredient extends Ingredient {
    ...
    public static final Codec<CountableIngredient> CODEC = Codec.either(CODEC_TAGGED_ITEM, CODEC_DIRECT_ITEM)
	    .xmap(either -> either.map(tag -> tag, item -> item), value -> {
		//
		...
		if (value.tag != null) {
		    ...
		    throw new UnsupportedOperationException(
			    "The Countable Ingredient neither has a tag nor a direct item value defined!");
		}
	...
	public CountableIngredient decode(FriendlyByteBuf buffer) {
		...
		return new CountableIngredient(ItemTags.create(StreamCodec.RESOURCE_LOCATION.decode(buffer)),
