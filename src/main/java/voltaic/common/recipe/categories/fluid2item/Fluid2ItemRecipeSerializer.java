...
public class Fluid2ItemRecipeSerializer<T extends Fluid2ItemRecipe> extends VoltaicRecipeSerializer<T> {
	...
	super(CodecUtils.composite(StreamCodec.RESOURCE_LOCATION, T::getId, FluidIngredient.LIST_STREAM_CODEC,
		T::getFluidIngredients, StreamCodec.ITEM_STACK, T::getItemRecipeOutput, StreamCodec.DOUBLE, T::getXp,
     ...
     * List<FluidIngredient> inputs = FluidIngredient.readList(buffer); ItemStack
     * output = buffer.readItem(); double experience = buffer.readDouble(); int
     * ticks = buffer.readInt(); double usagePerTick = buffer.readDouble();
