...
public class Fluid2FluidRecipeSerializer<T extends Fluid2FluidRecipe> extends VoltaicRecipeSerializer<T> {
	...
	super(CodecUtils.composite(StreamCodec.RESOURCE_LOCATION, T::getId, FluidIngredient.LIST_STREAM_CODEC,
		T::getFluidIngredients, StreamCodec.FLUID_STACK, T::getFluidRecipeOutput, StreamCodec.DOUBLE, T::getXp,
     ...
     * List<FluidIngredient> inputs = FluidIngredient.readList(buffer); FluidStack
     * output = buffer.readFluidStack(); double experience = buffer.readDouble();
     * int ticks = buffer.readInt(); double usagePerTick = buffer.readDouble();
     * List<ProbableItem> itemBi = null; List<ProbableFluid> fluidBi = null;
     * List<ProbableGas> gasBi = null; if (hasItemBi) { itemBi =
