...
public class Fluid2GasRecipeSerializer<T extends Fluid2GasRecipe> extends VoltaicRecipeSerializer<T> {

    private final Fluid2GasRecipe.Factory<T> factory;

    public Fluid2GasRecipeSerializer(Fluid2GasRecipe.Factory<T> factory) {
	super(CodecUtils.composite(StreamCodec.RESOURCE_LOCATION, T::getId, FluidIngredient.LIST_STREAM_CODEC,
		T::getFluidIngredients, GasStack.STREAM_CODEC, T::getGasRecipeOutput, StreamCodec.DOUBLE, T::getXp,
		StreamCodec.INT, T::getTicks, StreamCodec.DOUBLE, T::getUsagePerTick, ProbableItem.LIST_STREAM_CODEC,
		T::getItemBiproducts, ProbableFluid.LIST_STREAM_CODEC, T::getFluidBiproducts,
		ProbableGas.LIST_STREAM_CODEC, T::getGasBiproducts, factory::create
    ...
    /*
     * @Override public @Nullable T fromNetwork(FriendlyByteBuf buffer) { String
     * group = buffer.readUtf(); boolean hasItemBi = buffer.readBoolean(); boolean
     * hasFluidBi = buffer.readBoolean(); boolean hasGasBi = buffer.readBoolean();
     * List<FluidIngredient> inputs = FluidIngredient.readList(buffer); GasStack
     * output = GasStack.readFromBuffer(buffer); double experience =
     ...
     * factory.create(group, inputs, output, experience, ticks, usagePerTick,
     * itemBi, fluidBi, gasBi); }
     * 
