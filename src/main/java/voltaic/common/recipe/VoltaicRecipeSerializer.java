...
public abstract class VoltaicRecipeSerializer<T extends VoltaicRecipe> implements RecipeSerializer<T> {
	...
	for (int i = 0; i < count; i++) {
		...
		throw new UnsupportedOperationException(
			recipeId.toString() + ": The count field does not match the input count");
	    }
	    CountableIngredient.CODEC.decode(JsonOps.INSTANCE, itemInputs.get(i + "")).result()
		    .ifPresent(pair -> itemIngredients.add(pair.getFirst()));
	}
	...
	for (int i = 0; i < count; i++) {
	    ...
	    FluidIngredient.CODEC.decode(JsonOps.INSTANCE, fluidInputs.get(i + "").getAsJsonObject()).result()
		    .ifPresent(pair -> fluidIngredients.add(pair.getFirst()));
	}
	...
	for (int i = 0; i < count; i++) {
	    ...
	    GasIngredient.CODEC.decode(JsonOps.INSTANCE, fluidInputs.get(i + "").getAsJsonObject()).result()
		    .ifPresent(pair -> gasIngredients.add(pair.getFirst()));
	}
	...
	for (int i = 0; i < count; i++) {
	    ...
	    ProbableItem.CODEC.decode(JsonOps.INSTANCE, itemBiproducts.get("" + i).getAsJsonObject()).result()
		    .ifPresent(pair -> items.add(pair.getFirst()));
	}
	return items;
    }
	...
	for (int i = 0; i < count; i++) {
	    ...
	    ProbableFluid.CODEC.decode(JsonOps.INSTANCE, fluidBiproducts.get(i + "").getAsJsonObject()).result()
		    .ifPresent(pair -> gases.add(pair.getFirst()));
	}
	return gases;
    }
	...
	for (int i = 0; i < count; i++) {
	    ...
	    ProbableGas.CODEC.decode(JsonOps.INSTANCE, gasBiproducts.get(i + "").getAsJsonObject()).result()
		    .ifPresent(pair -> fluids.add(pair.getFirst()));
	}
	return fluids;
    }
