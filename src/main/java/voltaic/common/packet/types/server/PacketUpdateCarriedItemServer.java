...
public class PacketUpdateCarriedItemServer {
	...
	public PacketUpdateCarriedItemServer decode(FriendlyByteBuf buffer) {
	    return new PacketUpdateCarriedItemServer(StreamCodec.ITEM_STACK.decode(buffer),
		    StreamCodec.BLOCK_POS.decode(buffer), StreamCodec.UUID.decode(buffer));
	}
    };
	...
	ctx.enqueueWork(() -> {
	    ServerBarrierMethods.handleUpdateCarriedItemServer(context.get().getSender().level(), message.carriedItem,
