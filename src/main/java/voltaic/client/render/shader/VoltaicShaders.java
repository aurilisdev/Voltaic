...
public class VoltaicShaders extends RenderType {
    ...
    // private static final ShaderStateShard RENDERTYPE_PLASMA_ORB = new
    // ShaderStateShard(() -> shaderPlasmaOrb);
    ...
    public static final RenderType GREATER_ALPHA = RenderType.create(GREATER_ALPHA_LOC.toString(),
	    ...
	    CompositeState.builder().setLightmapState(LIGHTMAP).setShaderState(RENDERTYPE_EYES_SHADER)
		    .setTextureState(NO_TEXTURE).setTransparencyState(RenderStateShard.TRANSLUCENT_TRANSPARENCY)
		    .createCompositeState(false));

    // @SubscribeEvent
    public static void onRegisterShaders(final RegisterShadersEvent event) {
	try {
	    event.registerShader(new ShaderInstance(event.getResourceProvider(), GREATER_ALPHA_LOC,
    ...
    private VoltaicShaders(String name, VertexFormat format, Mode mode, int bufferSize, boolean affectsCrumbling,
