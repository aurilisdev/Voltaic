...
public class ScreenGuidebook extends GenericScreen<ContainerGuidebook> {
	...
	blit(poseStack, guiWidth + LEFT_X_SHIFT, guiHeight, 0, 0, LEFT_TEXTURE_WIDTH, LEFT_TEXTURE_HEIGHT);

	RenderingUtils.bindTexture(PAGE_TEXTURE_RIGHT);
	blit(poseStack, guiWidth + RIGHT_X_SHIFT, guiHeight, 0, 0, RIGHT_TEXTURE_WIDTH, RIGHT_TEXTURE_HEIGHT);

	updatePageArrowVis();

	renderPageBackground(poseStack, LEFT_X_SHIFT, guiWidth, guiHeight, getCurrentPage());
	renderPageBackground(poseStack, RIGHT_X_SHIFT - 8, guiWidth, guiHeight, getNextPage());

    }

    private void renderPageBackground(PoseStack poseStack, int xShift, int guiWidth, int guiHeight, Page page) {

	for (GraphicWrapper graphic : page.graphics) {
	    graphic.graphic().render(poseStack, graphic.x(), graphic.y(), xShift, guiWidth, guiHeight, page);
	}
	...
	renderPageLabels(poseStack, LEFT_X_SHIFT, refX, refY, getCurrentPage());
	renderPageLabels(poseStack, RIGHT_X_SHIFT - 8, refX, refY, getNextPage());
	...
	renderPageTooltips(poseStack, LEFT_X_SHIFT, x, y, refX, refY, xAxis, yAxis, guiWidth, guiHeight,
		getCurrentPage());
	renderPageTooltips(poseStack, RIGHT_X_SHIFT - 8, x, y, refX, refY, xAxis, yAxis, guiWidth, guiHeight,
		getNextPage());

    }

    private void renderPageLabels(PoseStack poseStack, int xPageShift, int refX, int refY, Page page) {

	for (TextWrapper text : page.text) {

	    if (text.centered()) {
		int xShift = (TEXT_WIDTH - font.width(text.characters())) / 2;
		getFontRenderer().draw(poseStack, Language.getInstance().getVisualOrder(text.characters()),
			text.x() + refX + xShift + xPageShift, refY + text.y(), text.color().color());
	    } else {
		getFontRenderer().draw(poseStack, Language.getInstance().getVisualOrder(text.characters()),
			text.x() + refX + xPageShift, text.y() + refY, text.color().color());
	    }
	...
	for (GraphicWrapper wrapper : page.graphics) {
		...
		getFontRenderer().draw(poseStack, descriptor.text,
			refX + wrapper.x() + descriptor.xOffsetFromImage + xPageShift,
			refY + wrapper.y() + descriptor.yOffsetFromImage, descriptor.color);
	    }

	}

	page.renderAdditionalText(poseStack, refX, refY, xPageShift, font, TEXT_WIDTH, TEXT_START_X);

    }

    private void renderPageTooltips(PoseStack poseStack, int xPageShift, int mouseX, int mouseY, int refX, int refY,
	...
	for (TextWrapper text : page.tooltipText) {
	    ...
	    if (isPointInRegionText(refX + xShift + xPageShift + text.x(), refY + text.y(), xAxis, yAxis, textWidth,
		    LINE_HEIGHT)) {
		text.onTooltip().onTooltip(poseStack, xAxis, yAxis, this);
	    }
	...
	for (GraphicWrapper wrapper : page.tooltipGraphics) {
	    ...
	    if (isPointInRegionGraphic(mouseX, mouseY, guiWidth + wrapper.x() + graphic.lookupXOffset + xPageShift,
		    ...
		    graphic.height)) {
		wrapper.onTooltip().onTooltip(poseStack, xAxis, yAxis, this);
	    }
		...
		if (descriptor.onTooltip != null
		    ...
		    descriptor.onTooltip.onTooltip(poseStack, xAxis, yAxis, this);
		}
	...
	if (this.minecraft.options.keyInventory.isActiveAndMatches(mouseKey) && searchBox.isVisible()
		&& searchBox.isFocused()) {
	    return false;
	}
