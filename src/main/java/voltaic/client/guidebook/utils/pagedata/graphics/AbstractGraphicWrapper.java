...
/**
 * A specialized version of AbstractWrapperObject for drawing images amongst
 * other graphics to the guidebook screen. The image can also have text
 * associated with it.
 * 
 * @author skip999
 *
...
public abstract class AbstractGraphicWrapper<T extends AbstractGraphicWrapper<?>> extends AbstractWrapperObject<T> {
    ...
    public boolean allowNextToOthers = false; // allows the image to be placed horizontally next to another if space
					      // permits

    public final GraphicTextDescriptor[] descriptors;

    public AbstractGraphicWrapper(int xOffset, int yOffset, int lookupXOffset, int lookupYOffset, int width, int height,
	    int trueHeight, GraphicTextDescriptor... descriptors) {
    ...
    public abstract void render(GuiGraphics graphics, int wrapperX, int wrapperY, int xShift, int guiWidth,
	    int guiHeight, Page page);
