 ...
 * Guidebook screen The class is capable of displaying items/blocks as well as
 * text descriptors for said items/blocks
 * 
 * @author skip999
 *
 */
public class ItemWrapperObject extends AbstractGraphicWrapper<ItemWrapperObject> {
    ...
    public ItemWrapperObject(int xOffset, int yOffset, int width, int height, int trueHeight, float scale, Item item,
    ...
    public void render(GuiGraphics graphics, int wrapperX, int wrapperY, int xShift, int guiWidth, int guiHeight,
	    Page page) {
	RenderingUtils.renderItemScaled(graphics, item, guiWidth + xOffset + wrapperX + xShift,
		guiHeight + yOffset + wrapperY, scale);
    }
