...
public class Page {
	...
	graphics.drawString(font, moduleTitle, refX + textStartX + xShift + xPageShift, refY + 16,
	...
	graphics.drawString(font, chapTitle, refX + textStartX + xShift + xPageShift, refY + 26,
		Color.TEXT_GRAY.color(), false);

	Component pageNumber = Component.literal(getPage() + 1 + "");
	xShift = (textWidth - font.width(pageNumber)) / 2;
	graphics.drawString(font, pageNumber, refX + textStartX + xShift + xPageShift, refY + 200,
		Color.TEXT_GRAY.color(), false);
	...
	@Override
	public void renderAdditionalText(GuiGraphics graphics, int refX, int refY, int xPageShift, Font font,
		int textWidth, int textStartX) {

	    Module currMod = associatedModule;
	    Component moduleTitle = currMod.getTitle().withStyle(ChatFormatting.BOLD);
	    int xShift = (textWidth - font.width(moduleTitle)) / 2;
	    graphics.drawString(font, moduleTitle, refX + xShift + textStartX + xPageShift, refY + 16,
		    Color.TEXT_GRAY.color(), false);

	    Component chapTitle = VoltaicTextUtils.guidebook("chapters").withStyle(ChatFormatting.UNDERLINE);
	    xShift = (textWidth - font.width(chapTitle)) / 2;
	    graphics.drawString(font, chapTitle, refX + textStartX + xShift + xPageShift, refY + 31,
		    Color.TEXT_GRAY.color(), false);
	}
	...
	public void renderAdditionalText(GuiGraphics graphics, int refX, int refY, int xPageShift, Font font,
	    ...
	    graphics.drawString(font, modTitle, refX + textStartX + xShift + xPageShift, refY + 16,
		    Color.TEXT_GRAY.color(), false);
	}
