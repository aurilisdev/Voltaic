...
public class ScreenHandlerGuidebook implements IGuiContainerHandler<ScreenGuidebook> {
    ...
    @Override
    public Optional<IClickableIngredient<?>> getClickableIngredientUnderMouse(ScreenGuidebook screen, double mouseX,
	    double mouseY) {
    ...
    private static Optional<IClickableIngredient<?>> getJeiLookup(int xPageShift, int mouseX, int mouseY, int refX,
	    int refY, int xAxis, int yAxis, int guiWidth, int guiHeight, Page page, ScreenGuidebook screen) {
	...
	Rect2i area = new Rect2i(screen.getGuiLeft() + ScreenGuidebook.LEFT_X_SHIFT, screen.getGuiTop(),
		ScreenGuidebook.OLD_TEXTURE_WIDTH + ScreenGuidebook.RIGHT_TEXTURE_WIDTH,
		ScreenGuidebook.LEFT_TEXTURE_HEIGHT);

	// Rect2i area1 = new Rect2i(((int) (screen.getGuiLeft() +
	// screen.getGuiWidth())), screen.getGuiTop(), ((int)
	// (ScreenGuidebook.LEFT_X_SHIFT + ScreenGuidebook.LEFT_TEXTURE_WIDTH +
	// ScreenGuidebook.RIGHT_TEXTURE_WIDTH - screen.getGuiHeight())),
	// ScreenGuidebook.LEFT_TEXTURE_HEIGHT);
	// Rect2i area2 = new Rect2i(screen.getGuiLeft() + ScreenGuidebook.LEFT_X_SHIFT,
	// screen.getGuiTop(), -ScreenGuidebook.LEFT_X_SHIFT,
	// ScreenGuidebook.LEFT_TEXTURE_HEIGHT);
